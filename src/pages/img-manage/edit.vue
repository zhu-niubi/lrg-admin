...
export default {
  ...
  setup() {
    ...
    //
    const loadData = () => {
      if (JSON.stringify(imageItem) === '{}') {
        history.back();
      } else {
        imageItem.type = Number(imageItem.type);
        imageItem.url = [{ url: imageItem.url }];
        imageObj.value = imageItem;
      }
    };

    const postData = (data) => {
      // $q.loading.show()
      let id = imageObj.value.id;
      api
        .put(`/image/${id}`, data)
        .then((payload) => {
          $q.notify({
    ...
    const onSumbit = () => {
      myForm.value.validate().then((success) => {
        if (success) {
          ...
          newBodyData.url = bodyData.url[0] ? bodyData.url[0].url : '';

          // console.log(newBodyData);
          // return;
          if (!newBodyData || !newBodyData.url) {
            $q.notify({
              color: 'negative',
              position: 'top',
              message: '数据输入不完整!',
