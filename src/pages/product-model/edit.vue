<template>
  <q-page class="q-pa-md">
    ...
    <q-toolbar class="q-py-md">
      <q-toolbar-title>修改系列</q-toolbar-title>
    </q-toolbar>
    ...
    const onSumbit = () => {
      myForm.value.validate().then((success) => {
        if (success) {
          ...
          newBodyData.tag = bodyData.tag ? bodyData.tag : undefined;
          newBodyData.sort = Number(bodyData.sort);
          newBodyData.warrantyYear = Number(bodyData.warrantyYear) || undefined;

          if (
            !newBodyData ||
            !newBodyData.name ||
            !newBodyData.productTypeId ||
            !newBodyData.sort ||
            !newBodyData.warrantyYear
          ) {
            $q.notify({
              color: 'negative',
              position: 'top',
